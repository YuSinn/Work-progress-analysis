...
if input('Quieres introducir un nuevo mes? (Si/No) ') == 'Si':
    ...
    pass
mes_to_work = input('Que mes quieres editar? ') + '.csv'
        ...
        for i in range(1,dias+1):
            ...
            if mes in months:
                ...
                if dia in range(1,32):
                    ...
                    if(tran > 0):
                        weekDay = input("Introduce de que dia de la semana son las transcripciones")
                else:
                ...
                if dia in range(1,32):
                    ...
                    weekDay = input("Introduce de que dia de la semana son las transcripciones") 
            print('~~~ \n ~~~\n~~~')  
            writer.writerow([dia, mes, tran, weekDay])
    else:
        pass
...
def work_analisys(df):
    if work_df['Month'][0] == 'Agosto':
        ...
        print('1.) The average transcriptions for ' + str(work_df['Month'][0]) + ' is ' + str((int(round(np.average(work_df['Transcriptions']),0))))+'.')
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('2.) Total transcriptions in ' + str(work_df['Month'][0]) +' is ' +str(np.sum(work_df['Transcriptions'])) +' of ' + str(2227*np.count_nonzero(work_df[work_df['WeekDay'] != 'Friday']['Transcriptions'])+1890*np.count_nonzero(work_df[work_df['WeekDay'] == 'Friday']['Transcriptions']))+'.')
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('3.) The objective of the month was surpased with ' + str((np.sum(work_df[work_df['WeekDay'] != 'Friday']['Transcriptions'])-2227*np.count_nonzero(work_df[work_df['WeekDay'] != 'Friday']['Transcriptions'])+np.sum(work_df[work_df['WeekDay'] == 'Friday']['Transcriptions'])-1890*np.count_nonzero(work_df[work_df['WeekDay'] == 'Friday']['Transcriptions']))) + ' transcriptions.')
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('4.) The revenue for ' + str(work_df.Month[0]) + ' is ' + str(round((np.sum(work_df[work_df['WeekDay'] != 'Friday']['Transcriptions'])-2227*np.count_nonzero((work_df[work_df['WeekDay'] != 'Friday']['Transcriptions']))+np.sum(work_df[work_df['WeekDay'] == 'Friday']['Transcriptions'])-1890*np.count_nonzero((work_df[work_df['WeekDay'] == 'Friday']['Transcriptions'])))*0.028,2)) +'€.')
